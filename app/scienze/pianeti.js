...
import { sistemaSolare } from '../../data/scienze';
...
export default function Pianeti() {
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top + 16 }]}>
      ...
      <Text style={styles.title}>🪐 Il Sistema Solare</Text>
      <View style={styles.dots}>
        {sistemaSolare.map((_, i) => <View key={i} style={[styles.dot, i === idx && styles.dotActive]} />)}
      </View>
      ...
      <View style={styles.nav}>
        ...
        <Text style={styles.counter}>{idx + 1}/{sistemaSolare.length}</Text>
        <TouchableOpacity style={[styles.navBtn, idx === sistemaSolare.length - 1 && styles.navBtnDisabled]} onPress={() => idx < sistemaSolare.length - 1 && goTo(idx + 1)} activeOpacity={0.7}>
          <Text style={styles.navBtnText}>→</Text>
        </TouchableOpacity>
      </View>
    </View>
...
const styles = StyleSheet.create({
  ...
  name: { fontSize: 28, fontWeight: '800', color: COLORS.text, textAlign: 'center', marginBottom: 4 },
  type: { fontSize: 14, color: SCI_COLOR, textAlign: 'center', fontWeight: '600', marginBottom: 16 },
  ...
  navBtn: { backgroundColor: SCI_LIGHT, borderRadius: 16, paddingVertical: 14, paddingHorizontal: 24, alignItems: 'center' },
