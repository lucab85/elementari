...
export default function SciQuiz() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const [questions] = useState(() => [...scienzeQuiz].sort(() => Math.random() - 0.5));
...
const styles = StyleSheet.create({
  ...
  scoreLabel: { fontSize: 22, fontWeight: '700', color: SCI_COLOR },
  title: { fontSize: 28, fontWeight: '800', color: SCI_COLOR, marginBottom: 12 },
  ...
  doneText: { fontSize: 36, fontWeight: '800', color: SCI_COLOR, marginTop: 16 },
  doneScore: { fontSize: 20, color: COLORS.textLight, marginTop: 8 },
  retryBtn: { backgroundColor: SCI_COLOR, borderRadius: 16, paddingHorizontal: 40, paddingVertical: 16, marginTop: 24 },
