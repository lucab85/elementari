...
import { geografiaQuiz } from '../../data/geografia';
...
export default function GeoQuiz() {
  ...
  const [questions] = useState(() => [...geografiaQuiz].sort(() => Math.random() - 0.5));
  ...
  return (
    <ScrollView style={[styles.container, { paddingTop: insets.top }]} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.title}>🌍 Quiz Geografia</Text>
...
const styles = StyleSheet.create({
  ...
  scoreLabel: { fontSize: 22, fontWeight: '700', color: GEO_COLOR },
  title: { fontSize: 28, fontWeight: '800', color: GEO_COLOR, marginBottom: 12 },
  ...
  options: { gap: 10 },
  optionBtn: { backgroundColor: COLORS.white, borderRadius: 16, padding: 20, alignItems: 'center', borderWidth: 2, borderColor: GEO_LIGHT },
  optionCorrect: { backgroundColor: '#D5F5E3', borderColor: COLORS.correct },
  optionFaded: { opacity: 0.4 },
  optionText: { fontSize: 19, fontWeight: '700', color: GEO_COLOR },
  doneText: { fontSize: 36, fontWeight: '800', color: GEO_COLOR, marginTop: 16 },
