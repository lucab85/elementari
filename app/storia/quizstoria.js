...
export default function QuizStoria() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const [questions] = useState(() => [...storiaQuiz].sort(() => Math.random() - 0.5));
...
const styles = StyleSheet.create({
  ...
  title: { fontSize: 28, fontWeight: '800', color: STORIA_COLOR, marginBottom: 8 },
  ...
  doneText: { fontSize: 28, fontWeight: '800', color: STORIA_COLOR, marginTop: 16 },
  retryBtn: { backgroundColor: STORIA_COLOR, borderRadius: 16, paddingHorizontal: 40, paddingVertical: 16, marginTop: 24 },
