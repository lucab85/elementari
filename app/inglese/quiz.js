...
import { vocabulary, generateEnglishQuiz } from '../../data/inglese';

const ENG_COLOR = '#E67E22';
const ENG_LIGHT = '#FDEBD0';
const QUIZ_SIZE = 15;

export default function EngQuiz() {
  const router = useRouter();
  const insets = useSafeAreaInsets();
  const [questions] = useState(() => generateEnglishQuiz(vocabulary).slice(0, QUIZ_SIZE));
  ...
  return (
    <ScrollView style={[styles.container, { paddingTop: insets.top }]} contentContainerStyle={styles.content}>
      ...
      <Text style={styles.title}>🇬🇧 Quiz Inglese</Text>
      ...
      {feedback === 'wrong' && <Text style={styles.wrongText}>❌ It was: {q.options[q.correct]}</Text>}
...
const styles = StyleSheet.create({
  ...
  scoreLabel: { fontSize: 22, fontWeight: '700', color: ENG_COLOR },
  title: { fontSize: 28, fontWeight: '800', color: ENG_COLOR, marginBottom: 12 },
  progressBar: { flexDirection: 'row', justifyContent: 'center', gap: 4, marginBottom: 20, flexWrap: 'wrap' },
  dot: { width: 8, height: 8, borderRadius: 4, backgroundColor: '#E0E0E0' },
  dotActive: { backgroundColor: ENG_COLOR, width: 20 },
  dotDone: { backgroundColor: '#2ECC71' },
  questionBox: { backgroundColor: COLORS.white, borderRadius: 20, padding: 24, marginBottom: 20, alignItems: 'center',
    shadowColor: '#000', shadowOffset: { width: 0, height: 4 }, shadowOpacity: 0.1, shadowRadius: 12, elevation: 5 },
  questionEmoji: { fontSize: 48, marginBottom: 12 },
  questionText: { fontSize: 20, fontWeight: '600', color: COLORS.text, lineHeight: 28, textAlign: 'center' },
  ...
  options: { gap: 10 },
  optionBtn: { backgroundColor: COLORS.white, borderRadius: 16, padding: 18, alignItems: 'center', borderWidth: 2, borderColor: ENG_LIGHT },
  optionCorrect: { backgroundColor: '#D5F5E3', borderColor: COLORS.correct },
  optionFaded: { opacity: 0.4 },
  optionText: { fontSize: 20, fontWeight: '700', color: ENG_COLOR },
  doneText: { fontSize: 36, fontWeight: '800', color: ENG_COLOR, marginTop: 16 },
