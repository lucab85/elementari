...
function generate() {
  ...
  let a, b;
  if (level < 0.4) { a = Math.floor(Math.random() * 9) + 2; b = Math.floor(Math.random() * 90) + 10; }
  else if (level < 0.7) { a = Math.floor(Math.random() * 90) + 10; b = Math.floor(Math.random() * 90) + 10; }
  else { a = Math.floor(Math.random() * 900) + 100; b = Math.floor(Math.random() * 90) + 10; }
  return { a, b, answer: a * b };
}

export default function Moltiplicazioni() {
  ...
  return (
    <KeyboardAvoidingView style={[styles.container, { paddingTop: insets.top + 16 }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      ...
      <Text style={styles.title}>🔢 Moltiplicazioni</Text>
...
const styles = StyleSheet.create({
  ...
  questionText: { fontSize: 40, fontWeight: '800', color: COLORS.text },
