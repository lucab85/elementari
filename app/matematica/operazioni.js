  ...
  return (
    <KeyboardAvoidingView style={[styles.container, { paddingTop: insets.top + 16 }]} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <View style={styles.header}>
        ...
        <Animated.Text style={[styles.scoreText, { transform: [{ scale: bounceAnim }] }]}>
          ⭐ {score}/{total}
        </Animated.Text>
      </View>
      ...
      {feedback === 'wrong' && <Text style={styles.wrongText}>❌ La risposta era {q.answer}</Text>}
...
const styles = StyleSheet.create({
  ...
  input: {
    backgroundColor: COLORS.white, borderRadius: 16, padding: 20, fontSize: 28,
    textAlign: 'center', fontWeight: '700', marginBottom: 16,
    borderWidth: 2, borderColor: COLORS.mathLight,
  },
  checkBtn: {
    backgroundColor: COLORS.math, borderRadius: 16, padding: 18, alignItems: 'center',
  },
